...
/** signum(argument). Uses PureTransformer batched fillArray. */
public final class Signum implements DensityFunctions.PureTransformer {
    private final DensityFunction df;

    private static final MapCodec<Signum> MAP_CODEC = RecordCodecBuilder.mapCodec(instance ->
            instance.group(
                    DensityFunction.HOLDER_HELPER_CODEC.fieldOf("argument").forGetter(Signum::df)
            ).apply(instance, Signum::new));
    public static final KeyDispatchDataCodec<Signum> CODEC = KeyDispatchDataCodec.of(MAP_CODEC);

    public Signum(DensityFunction df) {
        this.df = df;
    }
